...
interface MechanismSectionProps {
  content?: Partial<Omit<MechanismSectionContent, "steps">> & {
    steps?: MechanismStep[];
  };
}

const defaultSteps: Required<MechanismStep>[] = [
  {
    id: 1,
  ...
  },
  {
    ...
    description:
      "Dual-track design: defect repair (by question type) + variance control (pacing, stamina, decision boundaries).",
    details: [
  ...
  },
  {
    ...
    description:
      "Continuous monitoring of session logs and tutor performance; bold decisions when needed.",
    details: [
  ...
  },
  {
    ...
    description:
      "Not a grind—an orchestration checkpoint that desensitizes anxiety and validates strategy.",
    details: [
...
const defaultContent: MechanismSectionContent = {
  sectionTitle: "How our system actually works",
  sectionSubtitle: 'A governed system, not "more explanations."',
  steps: defaultSteps,
  closingTitle: "Your Progress, Continuously Protected",
  closingDescription:
...
export function MechanismSection({ content }: MechanismSectionProps) {
  const [activeStep, setActiveStep] = useState<number | null>(null);

  const mechanismContent: MechanismSectionContent = {
    ...defaultContent,
    ...content,
    steps: content?.steps ?? defaultContent.steps,
  };

  const steps = mechanismContent.steps.map((step, index) => {
    const fallback = defaultSteps[index] ?? defaultSteps[defaultSteps.length - 1];
    return {
      id: step.id ?? fallback.id ?? index + 1,
      icon: step.icon ?? fallback.icon,
      title: step.title ?? fallback.title,
      description: step.description ?? fallback.description,
      details: step.details ?? fallback.details,
    };
  });
    ...
    <section id="mechanism" className="py-20 bg-white">
      <div className="container mx-auto px-6 lg:px-8 max-w-6xl">
        <div className="text-center space-y-6 mb-16">
          <h2 style={{ color: "var(--primary-navy)" }}>{mechanismContent.sectionTitle}</h2>
          <p className="text-lg max-w-3xl mx-auto" style={{ color: "var(--neutral-600)" }}>
            {mechanismContent.sectionSubtitle}
          </p>
        </div>
          ...
          {steps.map((step, index) => (
            ...
            >
              <Card
                ...
                onClick={() => setActiveStep(activeStep === step.id ? null : step.id ?? null)}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <div className="flex items-center space-x-3">
                      <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                        <Badge className="w-8 h-8 p-0 flex items-center justify-center border-accent text-accent" variant="outline">
                          {step.id}
                        </Badge>
                      </motion.div>
                      <motion.div
                  ...
                  <motion.div animate={{ rotate: activeStep === step.id ? 90 : 0 }} transition={{ duration: 0.2 }}>
                    <ChevronRight className="w-5 h-5" style={{ color: "var(--accent-teal)" }} />
                  </motion.div>
                </div>
          ...
          <div className="flex items-start space-x-4">
            ...
            <div>
              <h4 className="font-medium mb-2" style={{ color: "var(--primary-navy)" }}>
                {mechanismContent.closingTitle}
              </h4>
              <p style={{ color: "var(--neutral-700)" }}>{mechanismContent.closingDescription}</p>
            </div>
          </div>
        </motion.div>
      </div>
